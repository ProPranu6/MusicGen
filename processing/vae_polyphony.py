...
def make_dataset_for_vae(track, resolution, batch_size, seq_len):
    """
    ...
    try:
        ...
        track = track.binarize().set_resolution(resolution).pad_to_same().stack() #(5, time_steps, 128) -> (none, seq_len, 128, 5)
        nimgs = track[1] - track[1]%seq_len
        input_dataset = tf.data.Dataset().from_tensor_slices(track[:, :nimgs, :].reshape(-1, seq_len, 128, track.shape[0]), batch_size=batch_size)
...
def make_dataset_for_music_gen(track, resolution, vae_model, batch_size, seq_len):
    try:
        # Parse track to get note and chord info
        track = track.binarize().set_resolution(resolution).pad_to_same().stack() #(5, time_steps, 128) -> (none, seq_len, 128, 5)
        nimgs = track[1] - track[1]%seq_len
        inputs = track[:, :nimgs, :].reshape(-1, seq_len, 128, track.shape[0])
        mean, logvar = vae_model.encode(inputs)
        embeds = np.concatenate([mean, logvar], axis=-1)
        input_embeds = embeds[:-1]
        output_embeds = embeds[1:]
        input_dataset = timeseries_dataset_from_array(input_embeds, None, sequence_length=seq_len, sequence_stride=1, batch_size=batch_size)
        output_dataset = timeseries_dataset_from_array(output_embeds, None, sequence_length=seq_len, sequence_stride=1, batch_size=batch_size)
        dataset = zip(input_dataset, output_dataset)
...
def sample_dataset_for_vae(dir, nsamples, train_size=0.8, val_size=0.2, seq_len=32, resolution=24, batch_size=64):
    """
    ...
    for dataset_type in ['train', 'val', 'test']:
        ...
        for trackid in tqdm(range(len(samples)),desc=f'Preparing {dataset_type} dataset...'):
            ...
            dataset = make_dataset_for_vae(track, resolution, batch_size, seq_len, )
            if dataset:
...
def sample_dataset_for_music_gen(dir, nsamples, train_size=0.8, val_size=0.2, seq_len=100, resolution=8, batch_size=64):
    samples = os.listdir(dir)[:nsamples]
    ...
    for dataset_type in ['train', 'val', 'test']:
        ...
        for trackid in tqdm(range(len(samples)),desc=f'Preparing {dataset_type} dataset...'):
            track = ppr.load(os.path.join(dir, samples[trackid]))
            dataset = make_dataset_for_music_gen(track, resolution, batch_size, seq_len)
            if dataset:
                store_batched_dataset(dataset, dataset_name=f'Pr{os.path.basename(dir)}-Tr{trackid}', dataset_type=dataset_type)
...
def sample_track_for_vae(dir, nsamples, seq_len, resolution):
    """
    ...
    for trackid in tqdm(range(0, nsamples, 1), desc='Sampling tracks...'):
        ...
        input_track = track[:, :seq_len, :].reshape(-1, seq_len, 128, track.shape[0])
        output_track = track[:, seq_len:, :]

        tracks += [(input_track, output_track)]
        
    return tracks

def sample_track_for_music_gen(dir, nsamples, seq_len=100, resolution=8, vae_model=None):
    """
    Creates a set of input-output tuples of sequences following the input sequence length and resolution 
    for tracks taken from the directory, sampling a total of nsamples.
    ...
    for trackid in tqdm(range(0, nsamples, 1), desc='Sampling tracks...'):
        ...
        mean, logvar = vae_model.encode(inputs)
        embeds = np.concatenate([mean, logvar], axis=-1)
        input_embeds = embeds[:seq_len]
        output_embeds = embeds[seq_len:]
        tracks += [(input_embeds, output_embeds)]
